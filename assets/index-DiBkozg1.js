...
`}function g(){let e=document.getElementById(`hero-box`);e&&requestAnimationFrame(()=>{e.classList.remove(`opacity-0`,`translate-y-4`),e.classList.add(`opacity-100`,`translate-y-0`)})}function _(e){return e===1?1:1-2**(-10*e)}function v(e){let t=document.getElementById(`hero-count`);if(!t)return;let n=performance.now();function r(i){let a=Math.min((i-n)/1800,1);t.textContent=Math.floor(e*_(a)).toLocaleString(),a<1?requestAnimationFrame(r):t.textContent=e.toLocaleString()}requestAnimationFrame(r)}var y=`/thdrc-rebuild/assets/director-DG6dCxlH.png`,b=`/thdrc-rebuild/assets/thdrc_administration-building-NdksYf7c.png`,x=`/thdrc-rebuild/assets/threads-yrbCapJs.png`,S=`/thdrc-rebuild/assets/discord-DGt7Cphn.png`;function te(){let e=a()===`zh`;return setTimeout(()=>{let e=document.getElementById(`about`);e&&requestAnimationFrame(()=>{e.classList.remove(`opacity-0`,`translate-y-6`),e.classList.add(`opacity-100`,`translate-y-0`)})},100),setTimeout(()=>{let e=document.getElementById(`director-text`),t=document.getElementById(`history-text`);e&&w(e,e.dataset.text||``),t&&w(t,t.dataset.text||``)},400),`
<section
id="about"
class="
...
<div class="max-w-5xl mx-auto px-4 md:px-6">
  ...
  ">
    ...
    <div class="flex flex-col md:flex-row gap-4 md:gap-6 items-start">
      <img src="${y}" alt="Director" class="w-full md:w-48 h-auto object-cover rounded-lg" />
      <p
        id="director-text"
  ...
  ">
    ...
    <div class="flex flex-col md:flex-row gap-4 md:gap-6 items-start">
      <img src="${b}" alt="THDRC Building" class="w-full md:w-48 h-auto object-cover rounded-lg" />
      <p
        id="history-text"
  ...
  ">
    ...
    ${C(`📚`,r(`featureDatabase`),r(`featureDatabaseDesc`))}

    ${C(`🌐`,r(`featureBilingual`),r(`featureBilingualDesc`))}

    ${C(`🌙`,r(`featureTheme`),r(`featureThemeDesc`))}
  ...
  `}function w(e,t,n=18){if(!e)return;e._twCancel&&(e._twCancel.cancelled=!0);let r={cancelled:!1};e._twCancel=r,e.textContent=``;let i=0;function a(){r.cancelled||i<t.length&&(e.textContent+=t[i],i++,setTimeout(a,n))}a()}function T(t){let n=t.HDS_Code||`***`;return`
    <div class="
  ...
  `}function E(e){switch((e||``).toLowerCase()){case`available`:return`bg-green-100 text-green-700`;case`eliminate`:return`bg-red-100 text-red-700`;default:return`bg-gray-100 text-gray-700`}}function D(e){switch(e){case`A+`:return`bg-yellow-200 text-yellow-900 font-bold`;case`A`:return`bg-emerald-100 text-emerald-700`;case`B`:return`bg-blue-100 text-blue-700`;case`C`:return`bg-amber-100 text-amber-700`;default:return`bg-gray-100 text-gray-700`}}function O(e){let t=(e||``).toLowerCase();return t.includes(`bias`)?`bg-purple-100 text-purple-700`:t.includes(`disinformation`)?`bg-pink-100 text-pink-700`:t.includes(`eliminate`)?`bg-red-100 text-red-700`:`bg-gray-100 text-gray-700`}var k={search:``,category:`ALL`,score:`ALL`,status:`ALL`,sort:`newest`,page:1,pageSize:12},A=!1,j=!1,M=null;function N(){return A?(j=!1,queueMicrotask(F)):(A=!0,queueMicrotask(F)),P()}function P(){return`
    <section class="
      p-6 text-[var(--text)]
      bg-[var(--bg)]
  ...
  `}async function F(){await p(),G(),queueMicrotask(()=>{I(),R(),U()})}function I(){j||(j=!0,L(),z(),ne(),B(),V(),H(),window.addEventListener(`themeChange`,G),window.addEventListener(`languageChange`,G))}function L(){let e=document.getElementById(`search`);e&&(e.oninput=e=>{clearTimeout(M),M=setTimeout(()=>{k.search=e.target.value.toLowerCase(),k.page=1,G()},150)})}function R(){let e=m();t(`category`,`Category`),t(`status`,`Status`),t(`score`,`Score`);function t(t,n){let r=document.getElementById(t);r&&(r.querySelectorAll(`option:not([value='ALL'])`).forEach(e=>e.remove()),[...new Set(e.map(e=>e[n]).filter(e=>e!=null&&e!==``).map(e=>String(e).trim()))].forEach(e=>{let t=document.createElement(`option`);t.value=e,t.textContent=e,r.appendChild(t)}))}}function z(){let e=(e,t)=>{let n=document.getElementById(e);n&&(n.onchange=e=>{k[t]=e.target.value,k.page=1,G()})};e(`category`,`category`),e(`status`,`status`),e(`score`,`score`)}function ne(){let e=document.getElementById(`sortBtn`);e&&(e.onclick=()=>{k.sort=k.sort===`newest`?`oldest`:`newest`,G()})}function B(){let e=document.getElementById(`hero-category`);e&&(e.onchange=e=>{k.category=e.target.value,k.page=1,G()})}function V(){let e=document.getElementById(`hero-search`);e&&(e.oninput=e=>{clearTimeout(M),M=setTimeout(()=>{k.search=e.target.value.toLowerCase(),k.page=1,G()},150)})}function H(){let e=document.getElementById(`hero-search-btn`);e&&(e.onclick=()=>{let e=document.getElementById(`hero-search`);e&&(k.search=e.value.toLowerCase(),k.page=1,G())})}function U(){let e=document.getElementById(`sortBtn`);e&&(e.innerHTML=`
    <span class="font-semibold">
      ⇅ ${k.sort===`newest`?`Newest`:`Oldest`}
    </span>
  `)}function W(){let e=m();return k.category!==`ALL`&&(e=e.filter(e=>e.Category===k.category)),k.status!==`ALL`&&(e=e.filter(e=>e.Status===k.status)),k.score!==`ALL`&&(e=e.filter(e=>(e.Score||``).startsWith(k.score))),k.search&&(e=e.filter(e=>(e.Title||``).toLowerCase().includes(k.search))),e.sort((e,t)=>{let n=Number(e.HDS_Code),r=Number(t.HDS_Code);return k.sort===`newest`?r-n:n-r})}function G(){let e=document.getElementById(`list`),t=document.getElementById(`papers-status`);if(!e)return;let n=W(),r=Math.max(1,Math.ceil(n.length/k.pageSize));k.page>r&&(k.page=1);let i=(k.page-1)*k.pageSize;e.innerHTML=n.slice(i,i+k.pageSize).map(T).join(``),t&&(t.textContent=`Loaded ${n.length} papers | Page ${k.page}/${r}`),K(r),U()}function K(e){let t=document.getElementById(`pagination`);t&&(t.innerHTML=q(k.page,e).map(e=>e===`...`?`<span class="px-2 opacity-50">...</span>`:`
      <button class="
        px-3 py-1 border rounded-lg
        bg-[var(--card)]
        border-[var(--border)]
        ${e===k.page?`font-bold`:`opacity-70`}
      ">
        ${e}
      </button>
    `).join(``),t.querySelectorAll(`button`).forEach(e=>{e.onclick=()=>{k.page=Number(e.textContent),G()}}))}function q(e,t){let n=[],r=Math.max(1,e-3),i=Math.min(t,e+3);r>1&&(n.push(1),r>2&&n.push(`...`));for(let e=r;e<=i;e++)n.push(e);return i<t&&(i<t-1&&n.push(`...`),n.push(t)),n}function J(){return`
    <footer class="
      bg-[var(--bg)]
      text-[var(--text)]
      border-t border-[var(--border)]
  ...
  `}var Y=`theme`;function X(){let e=localStorage.getItem(Y),t=window.matchMedia(`(prefers-color-scheme: dark)`).matches,n=e?e===`dark`:t;document.documentElement.classList.toggle(`dark`,n),localStorage.setItem(Y,n?`dark`:`light`)}function Z(e,t){let n=document.querySelector(e);if(!n){console.warn(`[mount] target not found: ${e}`);return}n.innerHTML=t}var re={home:h,research:N,about:te},Q=!1;function $(){if(Q)return;Q=!0,requestAnimationFrame(()=>{Q=!1});let e=re[location.hash.replace(`#/`,``)||`home`]||h;Z(`#navbar`,s()),Z(`#page-content`,e()),Z(`#footer`,J())}async function ie(){X(),await p(),$()}window.addEventListener(`hashchange`,$),window.addEventListener(`languageChange`,$),window.addEventListener(`themeChange`,$),ie();